...
最新情報はスタッフまでお問い合わせください。"""
    ...
    def _match_template(self, query: str, templates: Dict[str, str], platform: str) -> Optional[str]:
        """プラットフォーム別テンプレートマッチング（リッチメニュー対応版）"""
        query_lower = query.lower()

        # リッチメニュー対応のキーワードマッピング
        keyword_mapping: Dict[str, List[str]] = {
            # リッチメニュー項目（完全一致優先）
            "AI相談": ["🤖 ai相談", "ai相談"],
            "AI住まいサイト": ["🌐 ai住まいサイト", "ai住まいサイト"],
            "資料請求": ["📋 資料請求", "資料請求"],
            "展示場来場予約": ["📍 展示場来場", "展示場来場予約", "展示場来場　予約"],
            "資金計画": ["💰 資金計画", "資金計画"],
