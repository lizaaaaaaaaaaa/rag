...
async def liff_consent(_: Request):
    ...
    # ★ f-string を使わず、プレースホルダを後置換する方式に変更（{}/} のエスケープ不要）
    html = """<!doctype html>
<html lang="ja">
<head>
<meta charset="utf-8">
...
<div class="wrap">
  ...
  <!-- ✅ 同意チェック10項目 -->
  <div class="item"><label><input type="checkbox" id="c1"  checked> プライバシーポリシー</label></div>
  <div class="item"><label><input type="checkbox" id="c2"  checked> 利用規約</label></div>
  <div class="item"><label><input type="checkbox" id="c3"  checked> 入力が外部サービスへ送信される場合あり</label></div>
  <div class="item"><label><input type="checkbox" id="c4"  checked> AIの課題・限界の理解</label></div>
  <div class="item"><label><input type="checkbox" id="c5"  checked> Cookie等の利用</label></div>
  <div class="item"><label><input type="checkbox" id="c6"  checked> AI に個人情報は入力しないでください</label></div>
  <div class="item"><label><input type="checkbox" id="c7"  checked> この AI は住宅検討の参考用に設計された自動応答です</label></div>
  <div class="item"><label><input type="checkbox" id="c8"  checked> AI の回答は必ずしも正しいとは限りません。→ 確定案内はスタッフが行います</label></div>
  <div class="item"><label><input type="checkbox" id="c9"  checked> ご質問の内容により、AI の回答までお時間を頂戴する場合がございます</label></div>
  <div class="item"><label><input type="checkbox" id="c10" checked> ご使用の前に、必ず同意が必要となります。何卒ご理解賜りますようお願い申し上げます</label></div>
    ...
    try {
      ...
      const acRes = await fetch(API_BASE + '/line/after-consent', {
        ...
        body: JSON.stringify({
          ...
          source: 'liff_consent',
          consent_id: (saved && saved.consent_id) || 'unknown',
      ...
      if (!acRes.ok) {
        ...
        try { console.warn('after-consent non-200:', acRes.status, await acRes.text()); } catch(_) {}
      }
