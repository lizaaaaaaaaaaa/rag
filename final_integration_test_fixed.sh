#!/bin/bash
# final_integration_test_fixed.sh - 最終統合テスト・運用開始スクリプト（修正版）
...
# 2. 必要なAPIサービス確認
echo "🔍 必要なAPIサービス確認..."
REQUIRED_APIS=("cloudfunctions.googleapis.com" "cloudscheduler.googleapis.com" "pubsub.googleapis.com" "firestore.googleapis.com" "eventarc.googleapis.com")
API_TESTS_PASSED=0
...
for i in {1..3}; do
    ...
    if [[ "$CHAT_RESPONSE" != "ERROR" ]] && echo "$CHAT_RESPONSE" | grep -q "answer"; then
        RESPONSE_TIME=$(calculate_duration $START_TIME $END_TIME)
...
if [[ $RESPONSE_TESTS_PASSED -gt 0 ]]; then
    AVG_RESPONSE_TIME=$((TOTAL_RESPONSE_TIME / RESPONSE_TESTS_PASSED))
    echo "  📊 平均応答時間: ${AVG_RESPONSE_TIME}秒"
    
    # 10秒以内目標の達成確認（修正版）
    if [ $AVG_RESPONSE_TIME -le 10 ]; then
        echo "  🎯 目標達成: 10秒以内応答 ✅"
    else
        echo "  ⚠️ 目標未達: 10秒以内応答"
    fi
...
# 結果集計
SUCCESS_RATE=$(calculate_percentage $TESTS_PASSED $TESTS_TOTAL)
echo "📊 テスト結果: $TESTS_PASSED/$TESTS_TOTAL PASSED (${SUCCESS_RATE}%)"

if [[ $TESTS_PASSED -eq $TESTS_TOTAL ]]; then
    GRADE="🏆 A+ (完璧)"
    STATUS="🚀 本格運用開始可能"
elif [[ $TESTS_PASSED -ge $((TESTS_TOTAL * 90 / 100)) ]]; then
    GRADE="🥇 A (優秀)"
    STATUS="✅ 運用開始可能"
...
echo ""
echo "✅ **正常動作中の機能:**"
echo "   • Ultra Fast Web Chat (平均${AVG_RESPONSE_TIME:-0}秒応答)"
