#!/bin/bash
# deploy_cloud_functions_fixed.sh - Cloud Functions自動更新システムデプロイ（修正版）
...
echo "🚀 Cloud Functions自動更新システムデプロイ開始（修正版）"
echo "================================================"
...
# 3. requirements.txt作成（修正版）
echo "3️⃣ requirements.txt作成（修正版）..."
...
# Cloud Build Service Accountの権限確認
echo "  🔧 Cloud Build Service Account権限確認..."
gcloud projects add-iam-policy-binding ${PROJECT_ID} \
    --member="serviceAccount:190389115361-compute@developer.gserviceaccount.com" \
    --role="roles/cloudbuild.builds.builder" \
    --condition=None 2>/dev/null || echo "権限は既に設定済み"
...
echo ""
echo "  📋 関数詳細:"
gcloud functions describe ${FUNCTION_NAME} --region=${REGION} --format="value(name,status)"
...
echo "   ./final_integration_test_fixed.sh"
echo ""