...
# グローバル変数：ベクトルストアと RAG チェーンのテンプレートを保持
vectorstore = None
...
async def load_models_on_startup():
    """
    アプリ起動時に以下を順に実行して初期化します。
      1) llm_runner.load_llm() を呼び出して LLM インスタンスを取得
      2) rag.ingested_text.load_vectorstore() でベクトルストアをロード
      3) rag.ingested_text.get_rag_chain() で RAG チェーン (RetrievalQA など) を構築
    失敗した箇所があっても可能な限り空のベクトルストアで復旧し、
    print でどこが True/False かをすべて残します。
    """
    ...
    # 1) LLM をロード
    try:
        # ※ このタイミングで proxies 削除は llm_runner.py の中で行っています。
        from llm.llm_runner import load_llm

        llm, tokenizer, max_tokens = load_llm()
        llm_instance = llm  # LLM インスタンスを保存
        print(f">>> LLM loaded successfully: {type(llm).__name__}")
        if hasattr(llm, "invoke"):
            # invoke() が使える場合は簡易チェック
    ...
    try:
        ...
        if llm_loaded:
            # LLM は動いているが vectorstore が読めないときは空ベクトルストアを作成
    ...
    # 3) LLM と vectorstore の両方がロードできていれば RAG チェーンを構築
    if llm_loaded and vectorstore:
        try:
            from rag.ingested_text import get_rag_chain

            # get_rag_chain() の内部では再度 load_llm() が呼ばれるため、
            # 事前に proxies をクリアしている llm_runner.py の実装が前提です。
            ...
            # 代替チェーン：単純な RetrievalQA を作成（プロンプト指定あり）
            try:
