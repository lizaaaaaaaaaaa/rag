...
# 🔧 追加：伏字・テンプレ占位表現のブロック用パターン
PLACEHOLDER_RE = re.compile(r"(○○|〇〇|××|X{2,}|XXXX|TBD|未定|要確認|？？？|\?{2,}|＜.*?＞|ここに.*?を書く)")
...
class ResponseEnhancementService:
    ...
    def _is_richmenu_response(self, response: str) -> bool:
        ...
        for button in richmenu_buttons:
            if response.strip().startsWith(button) if hasattr(str, "startsWith") else response.strip().startswith(button):
                return True
    ...
    async def enhance_response(self, response: str, query: str, platform: str = "web",
        ...
        # ✅ 先頭で最低限のサニタイズ：伏字・占位語を除去（リッチメニューでも適用）
        response = PLACEHOLDER_RE.sub("（資料に記載なし）", response or "")
        ...
        try:
            ...
            if self._is_richmenu_response(response):
                ...
                # リッチメニュー応答は元のまま返す
                return {
                    "enhanced_response": response,  # そのまま保持（先頭サニタイズのみ済）
                    "original_response": response,
                    "quality_assessment": None,  # 評価スキップ
                    "enhancements_applied": ["placeholder_sanitized", "richmenu_preservation"],
            ...
            if not self._needs_enhancement(quality_assessment):
                return {
                    ...
                    "enhancements_applied": ["placeholder_sanitized"],
                    "processing_time": time.time() - start_time,
                    "improvement_score": 0.0
                }
            ...
            # 3. 段階的品質向上
            enhanced_response = response
            applied_enhancements = ["placeholder_sanitized"]
    ...
    def _assess_response_quality(self, response: str, query: str,
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enhancement_features": [
                ...
                "Richmenu response preservation"
            ]
        }
